...
- Las reglas de Firestore para Asignaciones deben permitir que Coordinacion Academica activa cree asignaciones propias (`createdBy == auth.uid`) sin depender de diferencias entre `assignedPrograms`, abreviaturas, nomenclaturas o codigos de programa; la validacion fina de programa se ejecuta en la interfaz antes del guardado.
